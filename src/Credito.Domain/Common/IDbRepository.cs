...
{
    public interface IDbRepository<TAggregate> where TAggregate : AggregateRoot
    {
        Task<IList<TAggregate>> FindAsync(Expression<Func<TAggregate, bool>> filter, int skip = 0, int take = 10, CancellationToken cancellationToken = default);
